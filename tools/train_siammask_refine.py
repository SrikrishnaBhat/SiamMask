...
def train(train_loader, model, optimizer, lr_scheduler, epoch, cfg):
    ...
    for iter, input in enumerate(train_loader):
        ...
        x = {
            ...
            'reverse_template': torch.autograd.Variable(input[8]).cuda(),
            'reverse_search': torch.autograd.Variable(input[9]).cuda(),
            'reverse_label_cls': torch.autograd.Variable(input[10]).cuda(),
            'reverse_label_loc': torch.autograd.Variable(input[11]).cuda(),
            'reverse_label_loc_weight': torch.autograd.Variable(input[12]).cuda(),
